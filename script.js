...
cAns = num;
var loksha = [...qs]; loksha.shift();
console.log(`${qs[0]} ${rightAns}`)
for(var i=0;i<4;i++)
{ if( i== num ){continue;}
console.log(`${loksha} len of lok : ${loksha.length} `)
var Rando = Math.floor(Math.random() * (loksha.length -1) );
let elem = loksha[Rando];
loksha = loksha.filter(e=> loksha.indexOf(e)  != Rando  );
var numQ = `#a${i}`; console.log(numQ);
